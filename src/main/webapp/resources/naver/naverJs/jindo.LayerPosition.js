...
htOption.sPosition = "outside-top-left" || "outside-top" || "outside-top-right" || "outside-right" || "outside-bottom-right" || "outside-bottom" || "outside-bottom-left" || "outside-left" || "inside-top-left" || "inside-top" || "inside-top-right" || "inside-right" || "inside-bottom-right" || "inside-bottom" || "inside-bottom-left" || "inside-left" || "inside-center" 
    ...
    $init: function(el, elLayer, htOption) {
        ...
        this._wfSetPosition = jindo.$Fn(function() {
            ...
            if (el && this._welLayer.visible() && this._el) {
                var htClientSize = $Document().clientSize();
                var htPosition = this.getCurrentPosition();
                this._nFixedLeft = (htClientSize.width <= this._welLayer.width() && htPosition.nLeft <= 0) ? 0 : -1;
                this._nFixedTop = (htClientSize.height <= this._welLayer.height()) ? htPosition.nTop : -1;

                if (this.fireEvent("beforeAdjust", {
                        elLayer: el,
                        htCurrentPosition: htPosition,
                        htAdjustedPosition: this._adjustPosition(htPosition)
                    })) {
                    this.setPosition();
...
oLayerPosition.setPosition({ nTop : 100, nLeft : 100 }); 
    ...
    setPosition: function(htPosition) {
        ...
        if (this.option("bAuto")) {
            ...
            htPosition.nLeft = this._nFixedLeft > -1 ? this._nFixedLeft : htPosition.nLeft;
            htPosition.nTop = this._nFixedTop > -1 ? this._nFixedTop : htPosition.nTop;
        }
...
(return value) htPosition = {
    ...
    _adjustPosition: function(htPosition) {
        ...
        for (var i = 0, htCandidatePosition;
            (htCandidatePosition = aCandidatePosition[i]); i++) {
