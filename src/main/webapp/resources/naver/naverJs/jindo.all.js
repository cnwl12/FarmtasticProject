...
결과 캐쉬를 사용중이면 캐쉬에 저장
    ...
    /**
@patch http://devcode.nhncorp.com/projects/jindo/issue/2955
 * @function
 * @name $$.test
...
<html>
	<head>
		...
		<script type="text/javascript" language="javascript" src="${pageContext.request.contextPath}/resources/js/jindo.all.js"></script>
...
jindo.$Ajax.prototype._onload = (function(isIE) {
    if (isIE) {
        return function() {
            arguments.callee.__name = "jindo.$Ajax._onload";
...
@patch http://devcode.nhncorp.com/projects/jindo/issue/2965
 */
...
jindo.$Json._oldToString = function(oObj) {
    var func = {
        ...
        o: function(o) {
            ...
            for (var x in o) {
                if (o.hasOwnProperty(x)) {
                    if (typeof o[x] == "undefined" || typeof o[x] == "function") continue;
...
jindo.$Element.prototype.attr = function(sName, sValue) {
    ...
    if (typeof sName == "object") {
        for (var k in sName) {
            if (sName.hasOwnProperty(k)) {
                //jindo버그 수정 : 박정기
                if (typeof(sName[k]) != "undefined" && sName[k] === null) {
                    e.removeAttribute(k);
                } else {
...
jindo.$Fn.prototype.bind = function() {
    ...
    var b = function() {
        arguments.callee.__name = "jindo.$Fn.bind"
...
jindo.$Fn.prototype.bindForEvent = function() {
    ...
    var b = function(e) {
        arguments.callee.__name = "jindo.$Fn.bindForEvent"
...
이벤트가 정의된 엘리먼트
    ...
    //이벤트 오류때문에 수정 window.event가 없는 경우도 있을 수 있음.
    if (e.currentTarget) {
        this.currentElement = e.currentTarget;
    } else if (window.event) {
        this.currentElement = window.event.currentTarget;
    }
...
/**
 * @patch checkbox & radio IE관련 버그 임시 패치
 * http://devcode.nhncorp.com/projects/jindo/issue/2900
 */
jindo.$Element.prototype.fireEvent = function(sEvent, oProps) {
...
/**
 * @patch 진도의 $Form.serialize() 메서드 사용시, disabled된 엘리먼트가 포함되는 버그로 인해 아래의 메서드들을 오버라이딩 함
 * http://devcode.nhncorp.com/tracker/?group_id=243&atid=453&func=detail&aid=2452
 */
jindo.$Form.prototype.value = function(sKey) {
        ...
        jindo.$A(aEls).forEach(function(o) {
            switch (o.type) {
                ...
                case 'select-multiple':
                    ...
                    if (sVal.constructor == Array) {
                        var waVal = jindo.$A(sVal);
                        for (var j = 0, len2 = o.options.length; j < len2; j++) {
                            o.options[j].selected = waVal.has(o.options[j].value);
                        }
                    } else {
                        for (var k = 0, len3 = o.options.length; k < len3; k++) {
                            if (o.options[k].value == sVal) {
                                nIndex = k;
                            }
                        }
                        o.selectedIndex = nIndex;
...
/**
 * @patch undefined 속성 삭제 되도록 수정
 * http://devcode.nhncorp.com/projects/jindo/issue/2936
 * @param oObj
 */
jindo.$Json._oldToString = function(oObj) {
    var func = {
...
for (var x in jindo) {
    for (var y in jindo[x].prototype) {
        if (y != "constructor" && typeof(jindo[x].prototype[y]) == "function") {
            jindo[x].prototype[y]["__name"] = "jindo." + x + "." + y;
        }
    }
}